...
def run_express_mtmc(cfg: CfgNode):
    ...
    #single_score = sum(single_score_list) / len(single_score_list)
    ...
    all_ground_truths_empty = all(all(os.path.getsize(gt) == 0 for gt in mot_conf.EVAL.GROUND_TRUTHS) for mot_conf in mot_configs)
    all_predictions_empty = all(all(os.path.getsize(pred) == 0 for pred in mot_conf.EVAL.PREDICTIONS) for mot_conf in mot_configs)
